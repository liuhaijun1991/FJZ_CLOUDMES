...
{
    public class I054CaptrueProcess : taskBase
    {
        public string BU = "";
        ...
        public override void init()
        {
            ...
            Output.UI = new I054CaptrueProcess_UI(this);
        }
                ...
                try
                {
                    ...
                    for (int n = 0; n < rsn.Count; n++)
                    {
                        ...
                        var log = SFCDB.ORM.Queryable<R_MES_LOG>()
                            .Where(t => t.PROGRAM_NAME == "MESInterface" && t.FUNCTION_NAME == "GenerateAS_Build_Data" && t.DATA1 == rsn[n].SN && t.DATA4 == "N")
                            .First();
                        if (log != null)
                        {
                            logs.Add(log);
        ...
        private List<R_SN> GetPendingI054()
        {
            ...
            var temp_cto = SFCDB.ORM.Ado.SqlQuery<R_SN>(@"
                        SELECT *
                         ...
                         WHERE EXISTS (SELECT 1
                           ...
                           AND EXISTS (SELECT 1 FROM R_WO_BASE WHERE CLOSED_FLAG = 0)
                           ...
                           AND NOT EXISTS (SELECT 1
                                  FROM r_mes_log l
                                 WHERE sn.sn = l.data1
                                   AND l.PROGRAM_NAME = 'MESInterface'
                                   AND l.FUNCTION_NAME = 'GenerateAS_Build_Data'
                                   AND l.data4 is null)
                           AND sn.valid_flag = 1",
                            new { Factory })
                        .ToList();
            ...
            var temp = SFCDB.ORM.Ado.SqlQuery<R_SN>(@"
                        SELECT sn.*
                          FROM r_sn sn, R_WO_BASE w, o_order_main wo, o_sku_config sku
                         WHERE sn.workorderno = wo.prewo
                           AND sn.workorderno = w.workorderno
            ...
            var orderlist = SFCDB.ORM.Queryable<O_ORDER_MAIN>().Where(t => IMesDbEx.OracleContain(t.PREWO, wolist)).ToList();
