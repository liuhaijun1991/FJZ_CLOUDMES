...
{
    public partial class FJZDownloadWO_UI : UserControl
    {

        FJZDownloadWO obj_down_wo = null;
        public FJZDownloadWO_UI(FJZDownloadWO obj_down)
        {
            InitializeComponent();
        ...
        private void FJZDownloadWO_UI_Load(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                var from = dateTimePickerWO.Value.Date;
                var to = dateTimePickerWOto.Value.Date;
                while (from <= to)
                {
                    obj_down_wo.downLoadDate = from.ToString("yyyy-MM-dd");
                    obj_down_wo.Start();
                    obj_down_wo.downLoadDate = "";
                    from = from.AddDays(1);
                }
