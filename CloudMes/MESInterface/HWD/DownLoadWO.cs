...
namespace MESInterface.HWD
{
    public class DownLoadWO:taskBase
    {
        ...
        {
            string lockIp = "";
            var IsRuning = synLock.IsLock("HWD_DownLoadWO", SFCDB, DB_TYPE_ENUM.Oracle, out lockIp);
            if (IsRuning)
            {
        ...
        private void CallRFC()
        {
            ...
            try
            {
                ...
                if (dtITAB.Rows.Count > 0 && dtITAB.Rows[0]["ERRMSG"].ToString()!="")
                {
                ...
                stationInterFace.HWDDOConvert(BU,"", arrayConvertWO, "Interface", ip, SFCDB, DB_TYPE_ENUM.Oracle, true, ref msg);
                //HWDDOConvert(wo, null, LoginUser.EMP_NO, IP, SFCDB, DBTYPE, ref msg);
                downLoadDate = "";
            }
            catch(Exception ex)
            {
        ...
        //    List<C_ROUTE_DETAIL> routeDetailList=null;
        //    List<C_KEYPART> keypartList=null;
        //    List<string> keypartIDList=null;
        //    List<C_ROUTE_DETAIL> linkStationList=null;
        ...
        //                R_MES_LOG deleteLog = SFCDB.ORM.Queryable<R_MES_LOG>().Where(r => r.DATA1 == wo && r.DATA3 == "DELETE"
        //                  && r.PROGRAM_NAME == "MESStation.Config.HWD.C7B5API" && r.FUNCTION_NAME == "CutWO").ToList().FirstOrDefault();
        //                //已經被刪除過的工單不再次轉
        //                if (deleteLog != null)
        //                {
        //                    continue;
        ...
        //                {
        //                    //HWT的工單類型通過工單前綴來判斷
        //                    rowWOType = WOType.GetWOTypeByWO_HWT(SFCDB, row["AUFNR"].ToString());
        //                }
        //                else
        //                {
        ...
        //                }
        //                C_ROUTE route = C_ROUTE.GetRouteBySkuIdAndWoType(Sku.ID, SFCDB, rowWOType.WORKORDER_TYPE);
        //                if (route == null)
        //                {
        ...
        //                //    throw new Exception(" the route of " + row["MATNR"].ToString() + " not exist");
        //                //}     
        //                routeDetailList = RouteDetail.GetByRouteIdOrderBySEQASC(route.ID, SFCDB);
        //                if (routeDetailList == null || routeDetailList.Count == 0)
        //                {
        //                    throw new Exception("get route detail fail by " + route.ID);
        //                }
