...
namespace MESInterface
{
    public class DnBase 
    {
        ...
        public DnBase(string bu,string plant,string dbStr,string cust)
        {
            ...
            try
            {
                ZRFC_SFC_NSG_0003 = new MESPubLab.SAP_RFC.ZRFC_SFC_NSG_0003(Bu);
            }
            catch (Exception ee)
            {
                throw ee;
        ...
        void RfcDataToModelList()
        {
            ...
            if (ZRFC_SFC_NSG_0003.GetTableValue("SD_TO_HEAD").Rows.Count == 0)
                return;
            if (ZRFC_SFC_NSG_0003.GetTableValue("SD_DN_DETAIL").Rows.Count == 0)
                return;
        ...
        public bool DownDnDataToMes(string dn,string to)
        {
            ...
            using (var db = OleExec.GetSqlSugarClient(this.DbStr,false))
            {
                var result = db.Ado.UseTran(() =>
                {
                    ...
                    db.Insertable<SD_CUSTMER_PO>(sdCustmerPoList.FindAll(x => soList.Contains(x.VBELN)).ToList()).ExecuteCommand();
                    db.Insertable<SD_CUSTOMER_PO_ITEM>(sdCustmerPoItemList.FindAll(x => soList.Contains(x.VBELN)).ToList()).ExecuteCommand();
                    db.Insertable<SD_CUSTOMER_SO>(sdCustmerSoList.FindAll(x => x.VBELN == dn).ToList()).ExecuteCommand();
                    db.Insertable<SD_DN_DETAIL>(sdDnDetailList.FindAll(x => x.VBELN == dn).ToList()).ExecuteCommand();
                    db.Insertable<SD_REPORT_DETAIL>(sdReportDetailList.FindAll(x => x.VBELN == dn).ToList()).ExecuteCommand();
                    db.Insertable<SD_TO_DETAIL>(sdToDetailList.FindAll(x => x.VBELN == dn).ToList()).ExecuteCommand();
                    db.Insertable<SD_TO_HEAD>(sdToHeadList.FindAll(x => x.TKNUM == to).ToList()).ExecuteCommand();
                    #endregion
                ...
                {
                    WriteLog.WriteIntoMESLog(this.DbStr, "MES", "Interface", "MESInterface.VERTIV.VertivDownloadDn",
                        "VertivDownloadDn", $@"Dn:{dn};Err:{result.ErrorMessage}", "", "SaveTo", dn, "", "system", "N");
                    return false;
                }
            }
        }
        ...
        public bool isShipOut(string dn)
        {
            using (var db = OleExec.GetSqlSugarClient(this.DbStr, false))
            {
                //var entity = db.Queryable<MESDataObject.Module.R_DN_STATUS>().With(SqlSugar.SqlWith.NoLock).Any(x => x.DN_FLAG != "0" && x.DN_NO== dn);
                var entity = db.Queryable<MESDataObject.Module.R_SHIP_DETAIL>().With(SqlSugar.SqlWith.NoLock).Any(x => x.DN_NO == dn);
                return entity;
            }
        }
        ...
        public List<C_SHIPPING_ROUTE_DETAIL> GetGtRouteByPlant(string customerCode, string plant)
        {
            List<C_SHIPPING_ROUTE_DETAIL> cShipRouteDetailList = new List<C_SHIPPING_ROUTE_DETAIL>();
            using (var db = OleExec.GetSqlSugarClient(DbStr, false))
            {
                cShipRouteDetailList = db.Queryable<C_SHIPPING_ROUTE_DETAIL, C_SHIP_CUSTOMER>((csrd, cs) =>
                cs.ROUTENAME == csrd.ROUTENAME && cs.CUSTOMERNAME == customerCode && cs.BILLTOCODE == plant).OrderBy((csrd) => csrd.SEQ, SqlSugar.OrderByType.Asc).Select((csrd) => csrd).ToList();
            }
            return cShipRouteDetailList;
        }
    }

    public class DnBase_HWT
    {
        public string Bu, Plant, DbStr, Cust, DnCreatDateTime;
        ...
        public DnBase_HWT(string bu, string plant, string dbStr, string cust)
        {
            Bu = bu; Plant = plant; DbStr = dbStr; Cust = cust;
            try
            {
        ...
        public bool DownDnDataToMes(string dn, string to)
        {
            ...
            using (var db = OleExec.GetSqlSugarClient(this.DbStr, false))
            {
                ...
                {
                    WriteLog.WriteIntoMESLog(this.DbStr, "MES", "Interface", "MESInterface.HWT.HWTDownloadDn",
                        "HWTDownloadDn", $@"Dn:{dn};Err:{result.ErrorMessage}", "", "SaveTo", dn, "", "system", "N");
                    return false;
                }
            }
        }
