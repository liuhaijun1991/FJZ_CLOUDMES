...
{
    public class JuniperRepairStation : MesAPIBase
    {
        #region APIInfo
        ...
        protected APIInfo FGetRepairAttachment = new APIInfo()
        {
            FunctionName = "GetRepairAttachment",
            Description = "GetRepairAttachment",
            Parameters = new List<APIInputInfo>()
            {
                new APIInputInfo() {InputName = "ACTIONID", InputType = "string", DefaultValue = "" }
            },
            Permissions = new List<MESPermission>() { }
        };
        #endregion

        public JuniperRepairStation()
        {
            ...
            this.Apis.Add(FGetRepairAttachment.FunctionName, FGetRepairAttachment);
        }

        public void GetRepairAttachment(Newtonsoft.Json.Linq.JObject requestValue, Newtonsoft.Json.Linq.JObject Data, MESStationReturn StationReturn)
        {
            OleExec SFCDB = null;
            try
            {
                ...
                var strsql = $@"select id, name , filename,usetype , '' as URL from r_file where index1 = '{ACTIONID}' and state = 1";
                var FileList = SFCDB.RunSelect(strsql);
                T_R_FILE TRF = new T_R_FILE(SFCDB, DB_TYPE_ENUM.Oracle);
                List<object> ret = new List<object>();
                for (int i = 0; i < FileList.Tables[0].Rows.Count; i++)
                {
                    var Name = FileList.Tables[0].Rows[i]["name"].ToString();
                    var UseType = FileList.Tables[0].Rows[i]["usetype"].ToString();
                    ...
                    string URL;
                    filePath += "\\" + RRF.NAME;
                    URL = "DOWNLOAD/" + RRF.NAME;
                    var aaa = RRF.FILENAME.Split('.');
                    
                    if (aaa.Length > 0)
                    {
                        filePath += ("." + aaa[aaa.Length - 1]);
                        URL += ("." + aaa[aaa.Length - 1]);
                    }
                    ...
                    FileList.Tables[0].Rows[i]["URL"] = filePath;
                    ret.Add(new
                    {
                        ID = FileList.Tables[0].Rows[i]["id"].ToString(),
                        name = FileList.Tables[0].Rows[i]["name"].ToString(),
                        filename = FileList.Tables[0].Rows[i]["filename"].ToString(),
                        usetype = FileList.Tables[0].Rows[i]["usetype"].ToString(),
                        URL = URL
                    });
                }
        ...
        public void SaveReplace(Newtonsoft.Json.Linq.JObject requestValue, Newtonsoft.Json.Linq.JObject Data, MESStationReturn StationReturn)
        {
            ...
            try
            {
                ...
                Newtonsoft.Json.Linq.JArray Attachment = null;
                try
                {
                    Attachment = (Newtonsoft.Json.Linq.JArray)Data["Attachment"];
                }
                catch
                { }
                ...
                T_R_REPAIR_PCBA_RELATIONSHIP t_r_repair_pcba_relationship = new T_R_REPAIR_PCBA_RELATIONSHIP(SFCDB, DBTYPE);
                ...
                string actionID = t_r_repair_failcode.GetNewID(BU, SFCDB);
                ...
                if (Attachment != null)
                {
                    ...
                    {
                        string strSql = $@"update SFCRUNTIME.R_FILE f set index1 = '{actionID}' where f.name = '{Attachment[i].ToString()}' and f.usetype='RepAttr'";
                ...
                #region 9. New Table to indicate PCBA was repaired or not
                if(is_pcba == "YES")
                {
                    var repairedSNExists= SFCDB.ORM.Queryable<R_REPAIR_PCBA_RELATIONSHIP>().Where(r => r.REPAIRED_SN == pcba_sn && r.WO_FLAG == "0").Any();
                    if (!repairedSNExists)
                    {
                        Row_R_REPAIR_PCBA_RELATIONSHIP ActnRow = (Row_R_REPAIR_PCBA_RELATIONSHIP)t_r_repair_pcba_relationship.NewRow();
                        ActnRow.ID = t_r_repair_failcode.GetNewID(BU, SFCDB); //New ID
                        ActnRow.REPAIR_ACTION_ID = actionID; //Gets ID ASSGNED in R_REPAIR_ACTION
                        ActnRow.PARENT_SN = repair_sn; //Parent Failed SN
                        ActnRow.REPAIRED_SN = pcba_sn; //Real Repair SN (Can be the same as above)
                        ActnRow.WO_FLAG = "0"; //When checkout, it will assign a new Virtual WO
                        string insertRelationship = SFCDB.ExecSQL(ActnRow.GetInsertString(DBTYPE));

                        if (insertRelationship != "1")
                        {
                            throw new Exception(insertRelationship);
                        }
                    }
                }
                #endregion
