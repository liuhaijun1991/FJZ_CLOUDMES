...
namespace MESStation.Config.HWD
{
  ...
  public class DELIVER_INFOAPI : MesAPIBase
    {
        ...
        /// <summary>
        /// 獲取用戶上傳的R_SN_DELIVER_INFO表信息
        /// </summary>
        public void SelectDELIVER_INFOList(Newtonsoft.Json.Linq.JObject requestValue, Newtonsoft.Json.Linq.JObject Data, MESStationReturn StationReturn)
        {
            OleExec SFCDB = null;
            var list = new List<R_SN_DELIVER_INFO>();
            try
            {
                SFCDB = this.DBPools["SFCDB"].Borrow();
                string ORDERNO = Data["ORDERNO"].ToString();
                string EDIT_EMP = Data["EDIT_EMP"].ToString();
                string EDIT_TIME = Data["EDIT_TIME"].ToString();
              

                if (!string.IsNullOrEmpty(ORDERNO) || !string.IsNullOrEmpty(EDIT_EMP) || !string.IsNullOrEmpty(EDIT_TIME))
                {
                    list = SFCDB.ORM.Queryable<R_SN_DELIVER_INFO>()
                    .Where(t => t.VALID_FLAG == "1")
                    .WhereIF(!string.IsNullOrEmpty(ORDERNO), t => t.ORDERNO == ORDERNO)
                    .WhereIF(!string.IsNullOrEmpty(EDIT_EMP), t => t.EDIT_EMP == EDIT_EMP)
                    .WhereIF(!string.IsNullOrEmpty(EDIT_TIME), t => t.EDIT_TIME == Convert.ToDateTime(EDIT_TIME))
                    .ToList();
                }
                else
                {
                    list = SFCDB.ORM.Queryable<R_SN_DELIVER_INFO>().Where(t => t.VALID_FLAG == "1").OrderBy(t => t.EDIT_TIME, OrderByType.Asc).Take(1000).ToList();
                }
        ...
        /// <summary>
        /// 上傳EXCEL內容到R_SN_DELIVER_INFO表信息
        /// </summary>
        public void UploadDELIVER_INFO(Newtonsoft.Json.Linq.JObject requestValue, Newtonsoft.Json.Linq.JObject Data, MESStationReturn StationReturn)
        {
            OleExec SFCDB = null;
            ...
            try
            {
                ...
                T_R_SN_DELIVER_INFO T_DELIVER = new T_R_SN_DELIVER_INFO(SFCDB, DBTYPE);
                Row_R_SN_DELIVER_INFO R_DELIVER = (Row_R_SN_DELIVER_INFO)T_DELIVER.NewRow();
                ...
                {
                    string IMEI = dataArray[i]["IMEI"].ToString();
                    string ORDERNO = dataArray[i]["订单号"].ToString();
                    string CARTON_SN = dataArray[i]["通用中箱号"].ToString();
                    string PALLET_SN = dataArray[i]["通用栈板号"].ToString();

                    R_SN_DELIVER_INFO TGMES = SFCDB.ORM.Queryable<R_SN_DELIVER_INFO>().Where(t => t.IMEI == IMEI && t.VALID_FLAG == "1").ToList().FirstOrDefault();
                    if (TGMES != null)
                    {
                        throw new Exception("附件中IMEI[" + IMEI + "]已存在SN_DELIVER_INFO表中！");
                    }


                    #region 寫入R_SN_DELIVER_INFO表
                    SYSDATE = T_DELIVER.GetDBDateTime(SFCDB);
                    R_DELIVER = (Row_R_SN_DELIVER_INFO)T_DELIVER.NewRow();
                    R_DELIVER.ID = T_DELIVER.GetNewID(BU, SFCDB);
                    R_DELIVER.IMEI = IMEI;
                    R_DELIVER.ORDERNO = ORDERNO;
                    R_DELIVER.CARTON_SN = CARTON_SN;
                    R_DELIVER.PALLET_SN = PALLET_SN;
                    R_DELIVER.VALID_FLAG = "1";
                    R_DELIVER.CHECK_FLAG = "0";
                    R_DELIVER.DELIVER_FLAG = "0";
                    R_DELIVER.CURRENT_STATION = "DATA_LOAD";
                    R_DELIVER.NEXT_STATION = "OQC_CHECK";
                    R_DELIVER.DATALOAD_EMP = LoginUser.EMP_NO;
                    R_DELIVER.DATALOAD_TIME = SYSDATE;
                    R_DELIVER.EDIT_EMP = LoginUser.EMP_NO;
                    R_DELIVER.EDIT_TIME = SYSDATE;
                    R_DELIVER.PLANT = "WDN1";
                   
                    SFCDB.ExecSQL(R_DELIVER.GetInsertString(DBTYPE));
                    #endregion
                }
                StationReturn.Status = StationReturnStatusValue.Pass;
        ...
        /// <summary>
        /// 根據ID刪除R_SN_DELIVER_INFO表信息
        /// </summary>
        public void DeleteDELIVER_INFO(Newtonsoft.Json.Linq.JObject requestValue, Newtonsoft.Json.Linq.JObject Data, MESStationReturn StationReturn)
        {
            OleExec SFCDB = null;
            try
            {
                SFCDB = this.DBPools["SFCDB"].Borrow();
                string IMEI = Data["IMEI"].ToString();
                string strRemark = Data["Remark"].ToString();

                List<R_SN_DELIVER_INFO> TGMESList = SFCDB.ORM.Queryable<R_SN_DELIVER_INFO>().Where(t => t.IMEI == IMEI && t.VALID_FLAG == "1").ToList();
                if (TGMESList == null || TGMESList.Count == 0)
                {
                    StationReturn.MessageCode = "MES00000007";
                    StationReturn.MessagePara.Add(IMEI);
                    StationReturn.Status = StationReturnStatusValue.Fail;
                }
                else
                {
                     
                        SFCDB.ORM.Updateable<R_SN_DELIVER_INFO>()
                        .SetColumns(t => new R_SN_DELIVER_INFO() { VALID_FLAG = "0", REMARK = strRemark, EDIT_EMP = LoginUser.EMP_NO, EDIT_TIME = GetDBDateTime() })
                        .Where(t => t.IMEI == IMEI && t.VALID_FLAG == "1").ExecuteCommand();

                        MESPubLab.WriteLog.WriteIntoMESLog(SFCDB, BU, "CloudMES", "MESStation.Config.HWD.DELIVER_INFOAPI", "DeleteDELIVER_INFO", IMEI + ", Remark:" + strRemark, "", LoginUser.EMP_NO);
