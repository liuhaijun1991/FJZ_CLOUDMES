...
namespace MESStation.Label.HWT
{
    ...
    class HwtAssyLabelMAC16 : LabelBase
    {
        #region 定義輸出
        ...
        LabelOutput O_PRINTSN = new LabelOutput() { Name = "PRINTSN", Type = LabelOutPutTypeEnum.String, Description = "", Value = "" };
        LabelOutput O_TMSN = new LabelOutput() { Name = "TMSN", Type = LabelOutPutTypeEnum.String, Description = "", Value = "" };
        
        #endregion 定義輸出

        public HwtAssyLabelMAC16()
        {
            Inputs.Add(I_SN);
        ...
        public override void MakeLabel(OleExec DB)
        {
            ...
            if (t_r_sn_mac.CheckExists(SN, "SN", "", DB))
            {
                #region 本工站是否要打印
                if (t_c_macprint_config.CheckExists(r_sn.SKUNO, "SN", r_sn.NEXT_STATION, DB))
                {
                    if (!t_r_sn_mac.CheckExists(SN, "SN", "CHECKNULL", DB))
                    {
                        throw new MESReturnMessage($@" 該條碼MAC未分配MAC16，請確認！{SN}");
                    }
                    r_sn_mac = t_r_sn_mac.LoadData(SN, "SN", "", DB);
                    O_MACSN.Value = r_sn_mac.SUBSN;
                    string printmac = r_sn_mac.SUBSN; 

                    if (t_r_sn_mac.CheckUnique(SN, "SN", printmac, DB))
                    {
                        throw new MESReturnMessage($@"Error,MAC SN重碼！{SN},{printmac}");
                    }
                    ...
                    {
                        O_PRINTSN.Value = printmac; 
                    }
                    O_TMSN.Value = printmac;
                }
