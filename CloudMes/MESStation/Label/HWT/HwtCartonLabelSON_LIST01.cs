...
namespace MESStation.Label.HWT
{
    ...
    /// </summary>
    class HwtCartonLabelSON_LIST01 : LabelBase
    {
        #region 定義輸出
        ...
        public HwtCartonLabelSON_LIST01()
        {
            Inputs.Add(I_SN);
        ...
        public override void MakeLabel(OleExec DB)
        {
            ...
            try
            {
                ...
                O_MAC.Value = MacList;
                if (SnList.Count!= MacList.Count)
                {
                    throw new System.Exception($@"獲取SN數量和MAC數量不相等,{SnList.Count},{MacList.Count}," + I_CAROTON.Value.ToString());
                }
            }
            catch(Exception)
            {
