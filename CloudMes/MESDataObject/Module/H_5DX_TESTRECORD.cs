...
{
    public class T_H_5DX_TESTRECORD : DataObjectTable
    {
        public T_H_5DX_TESTRECORD(string _TableName, OleExec DB, DB_TYPE_ENUM DBType) : base(_TableName, DB, DBType)
        {

        }
        public T_H_5DX_TESTRECORD(OleExec DB, DB_TYPE_ENUM DBType)
        {
            RowType = typeof(Row_H_5DX_TESTRECORD);
            TableName = "H_5DX_TESTRECORD".ToUpper();
            DataInfo = GetDataObjectInfo(TableName, DB, DBType);
        }
        public List<H_5DX_TESTRECORD> GetSNStatusBYSN(string SN, string Status, OleExec DB)
        {
            return DB.ORM.Queryable<H_5DX_TESTRECORD>().Where(t => t.SN == SN && t.STATUS == Status).OrderBy(t => t.TEST_START_TIME, SqlSugar.OrderByType.Desc).ToList();
        }
    }
    public class Row_H_5DX_TESTRECORD : DataObjectBase
    {
        public Row_H_5DX_TESTRECORD(DataObjectInfo info) : base(info)
        {

        }
        public H_5DX_TESTRECORD GetDataObject()
        {
            H_5DX_TESTRECORD DataObject = new H_5DX_TESTRECORD();
            ...
            DataObject.RETEST_PINS_QTY = this.RETEST_PINS_QTY;
            DataObject.NOT_RETEST_PINS_QTY = this.NOT_RETEST_PINS_QTY;
            DataObject.FAIL_RETEST_PINS_QTY = this.FAIL_RETEST_PINS_QTY;
        ...
        public double? RETEST_PINS_QTY
        {
            get
            {
                return (double?)this["RETEST_PINS_QTY"];
            }
            set
            {
                this["RETEST_PINS_QTY"] = value;
            }
        }
        public double? NOT_RETEST_PINS_QTY
        {
            get
            {
                return (double?)this["NOT_RETEST_PINS_QTY"];
            }
            set
            {
                this["NOT_RETEST_PINS_QTY"] = value;
            }
        }
        public double? FAIL_RETEST_PINS_QTY
        {
            get
            {
                return (double?)this["FAIL_RETEST_PINS_QTY"];
            }
            set
            {
                this["FAIL_RETEST_PINS_QTY"] = value;
            }
        }
    ...
    public class H_5DX_TESTRECORD
    {
        ...
        public double? RETEST_PINS_QTY { get; set; }
        public double? NOT_RETEST_PINS_QTY { get; set; }
        public double? FAIL_RETEST_PINS_QTY { get; set; }
