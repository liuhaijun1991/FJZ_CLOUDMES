...
{
    public class T_R_7B5_PLAN : DataObjectTable
    {
        public T_R_7B5_PLAN(string _TableName, OleExec DB, DB_TYPE_ENUM DBType) : base(_TableName, DB, DBType)
        {

        }
        public T_R_7B5_PLAN(OleExec DB, DB_TYPE_ENUM DBType)
        {
            RowType = typeof(Row_R_7B5_PLAN);
            TableName = "R_7B5_PLAN".ToUpper();
            DataInfo = GetDataObjectInfo(TableName, DB, DBType);
        }

        public List<R_7B5_PLAN> Get7B5Plan(OleExec sfcdb)
        {
            return sfcdb.ORM.Queryable<R_7B5_PLAN>().ToList();
        }

        public bool ItemExist(OleExec sfcdb, string item)
        {
            return sfcdb.ORM.Queryable<R_7B5_PLAN>().Any(r => r.ITEM == item);
        }

        public R_7B5_PLAN GetPlanObject(OleExec sfcdb, string item)
        {
            return sfcdb.ORM.Queryable<R_7B5_PLAN>().Where(r => r.ITEM == item).ToList().FirstOrDefault();
        }
    }
    public class Row_R_7B5_PLAN : DataObjectBase
    {
        public Row_R_7B5_PLAN(DataObjectInfo info) : base(info)
        {

        }
        public R_7B5_PLAN GetDataObject()
        {
            R_7B5_PLAN DataObject = new R_7B5_PLAN();
