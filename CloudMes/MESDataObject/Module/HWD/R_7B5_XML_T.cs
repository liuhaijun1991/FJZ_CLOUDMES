...
{
    public class T_R_7B5_XML_T : DataObjectTable
    {
        public T_R_7B5_XML_T(string _TableName, OleExec DB, DB_TYPE_ENUM DBType) : base(_TableName, DB, DBType)
        {

        }
        public T_R_7B5_XML_T(OleExec DB, DB_TYPE_ENUM DBType)
        {
            RowType = typeof(Row_R_7B5_XML_T);
            TableName = "R_7B5_XML_T".ToUpper();
            DataInfo = GetDataObjectInfo(TableName, DB, DBType);
        }

        public R_7B5_XML_T GetObjectByTaskNO(OleExec sfcdb, string task_no)
        {
            return sfcdb.ORM.Queryable<R_7B5_XML_T>().Where(r => r.TASK_NO == task_no).ToList().FirstOrDefault();
        }
        public bool IsExist(OleExec sfcdb, string task_no)
        {
            return sfcdb.ORM.Queryable<R_7B5_XML_T>().Any(r => r.TASK_NO == task_no);
        }
        public DataTable GetNoTransferList(OleExec sfcdb)
        {
            string sql = $@" SELECT DISTINCT task_no, item,
                      SUBSTR (change_information, 0, 25) change_information,
                      qty, TO_CHAR (start_date, 'YYYYMMDD') start_date,
                      TO_CHAR (complete_date, 'YYYYMMDD') complete_date,
                      item_version, description, rohs, product_line, po_flag,
                     to_char(lasteditdt,'yyyy/mm/dd hh24:mi:ss') as lasteditdt, model  FROM r_7b5_xml_t   WHERE transfer_flag = 'N' AND cancel_flag = 'N' ";
            return sfcdb.ExecuteDataTable(sql, CommandType.Text, null);
        }
    }
    public class Row_R_7B5_XML_T : DataObjectBase
    {
        public Row_R_7B5_XML_T(DataObjectInfo info) : base(info)
        {

        }
        public R_7B5_XML_T GetDataObject()
        {
            R_7B5_XML_T DataObject = new R_7B5_XML_T();
