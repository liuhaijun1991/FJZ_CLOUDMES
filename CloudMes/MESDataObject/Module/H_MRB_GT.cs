...
{
    public class T_H_MRB_GT : DataObjectTable
    {
        public T_H_MRB_GT(string _TableName, OleExec DB, DB_TYPE_ENUM DBType) : base(_TableName, DB, DBType)
        {
        ...
        {
            string strSql = $@" update h_mrb_gt set total_QTY=case when (total_QTY is null) then 1 else (total_QTY+1) end,EDIT_EMP=:userno,EDIT_TIME=sysdate where workorderno=:wono and SAP_FLAG='0'";
    ...
    public class Row_H_MRB_GT : DataObjectBase
    {
        public Row_H_MRB_GT(DataObjectInfo info) : base(info)
        {
