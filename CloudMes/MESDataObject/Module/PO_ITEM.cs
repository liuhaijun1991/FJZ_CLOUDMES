...
namespace MESDataObject.Module
{
    public class T_PO_ITEM : DataObjectTable
    {
        ...
        public PO_ITEM LoadSPECLABEL(string StrPO, OleExec DB)
        {
            PO_ITEM PO_ITEM = null;
            Row_PO_ITEM Row_PO_ITEM = (Row_PO_ITEM)NewRow();
            DataTable Dt = new DataTable();
            string StrSql = $@"select * from PO_ITEM where PO='{StrPO}'";
            Dt = DB.ExecSelect(StrSql).Tables[0];
            if (Dt.Rows.Count > 0)
            {
                Row_PO_ITEM.loadData(Dt.Rows[0]);
                PO_ITEM = Row_PO_ITEM.GetDataObject();
            }
