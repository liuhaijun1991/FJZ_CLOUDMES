using System;
using System.Collections;
using System.Collections.Generic;
using System.Text;
 
namespace DcnSfcModel
{
    /// <summary>
    /// 功能: 实体类 ()
    /// 创建人：Eden     
    /// 创建日期：2019/12/31    
    /// </summary>
    [Serializable]
    public class mfctbdailydetail
    {
        public mfctbdailydetail()
        {
			
        }
 
       #region  mfctbdailydetail实体
 
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string ctbdate {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string bomname {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string partno {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string purchaseno {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string podnno {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public int sdqty {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public int mpsqty {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public int ctbqty {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public int mrpqty {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public int totalpoqty {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public int totaldnqty {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string etadate {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string etddate {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public int qtyunit {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public int qtybase {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string qtyuom {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string parentpartno1 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string parentpartno2 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string parentpartno3 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string replacepartno1 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string replacepartno2 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string replacepartno3 {get; set;}
	
	   
	   #endregion
    }
}
 
 