using System;
using System.Collections;
using System.Collections.Generic;
using System.Text;
 
namespace DcnSfcModel
{
    /// <summary>
    /// 功能: 实体类 ()
    /// 创建人：Eden     
    /// 创建日期：2019/12/31    
    /// </summary>
    [Serializable]
    public class e_kanban_data_history
    {
        public e_kanban_data_history()
        {
			
        }
 
       #region  e_kanban_data_history实体
 
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string line {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string station {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string side {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string exception {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string skuname {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string skuno {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public decimal UPH {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public DateTime start1 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public DateTime end1 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public decimal period1 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public decimal output1 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public DateTime start2 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public DateTime end2 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public decimal period2 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public decimal output2 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string output_efficiency {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string efficiency_compass {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string result {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string remark {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string color1 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string color2 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string color3 {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public bool status {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public string lasteditby {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public DateTime lasteditdt {get; set;}
	
	
	   /// <summary>
	   /// 
	   /// </summary>
	   public DateTime refreshdt {get; set;}
	
	   
	   #endregion
    }
}
 
 