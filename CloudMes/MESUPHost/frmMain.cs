...
namespace MESCMCHost
{
    public partial class frmMain : Form
    {
        ...
        public frmMain()
        {
            ...
            try
            {
                //MESAPI.MESWebSocket = new WebSocket(MESConnStr);
                MESAPI.Connect();
                MESAPI.Login();
        ...
        void renewCmcUI()
        {
            ...
            {
                string id = HOSTINFO["CMCS"][i]["ID"].ToString();
                string ip = HOSTINFO["CMCS"][i]["IP"].ToString();
                string Line = HOSTINFO["CMCS"][i]["LINE"].ToString();
                string station = HOSTINFO["CMCS"][i]["STATION_NAME"].ToString();
                //CMC503_UI ui = new CMC503_UI();
                CMC503Scanda scanda = new CMC503Scanda(MESConnStr, id, ip, station, Line);
        ...
        {
            CMC503Scanda test = new CMC503Scanda(MESConnStr, "0001", "B223FC10", "ICT", "");
            test.IP = "10.120.156.11";
            test.Line = "B223FC10";
            test.Station = "ICT";
