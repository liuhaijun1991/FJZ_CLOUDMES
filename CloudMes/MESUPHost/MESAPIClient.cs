...
namespace MESCMCHost
{
    public class MESAPIClient
    {
        ...
        public void Connect()
        {
            if (this.MESWebSocket == null)
            {
                this.MESWebSocket = new WebSocket(MESConnStr);
                this.MESWebSocket.OnMessage += MESWebSocket_OnMessage;
                this.MESWebSocket.OnClose += MESWebSocket_OnClose;
            }
            try
            {
        ...
        public void ReSetApStation(string ID, string SN)
        {
            MESAPIData mesdata = new MESAPIData();
            mesdata.Class = "MESStation.Config.CMC.CMCConfig";
            mesdata.Function = "UndoDlStation";
            mesdata.Data = new { Station_No = ID, SN = SN };
            //CallMESAPI(mesdata, null, null);
            CallMESAPISync(mesdata, 1000);
