...
namespace MESCMCHost.Commamd
{
    ...
    public class inputStationName : CMCCommandAction
    {
        ...
        public void DoActionTest(CMC503Scanda Scanda, object data)
        {

            ((Station)comm).StationName = data.ToString();
            MESAPIData mesdata = new MESAPIData();
            mesdata.Class = "MESStation.Stations.CallStation";
            mesdata.Function = "InitStationForSMO";
            mesdata.Data = new { DisplayStationName = ((Station)comm).StationName, Line = Scanda.Line, Station_NO = Scanda.Id };
    ...
    public class inputStationInput : CMCCommandAction
    {
        ...
        public void DoActionTest(CMC503Scanda Scanda, object data)
        {
            ...
            MESAPIData mesdata = new MESAPIData();
            mesdata.Class = "MESStation.Stations.CallStation";
            mesdata.Function = "StationInputForSMO";
            // Cmd.JStation["ScanType"] = Cmd.ScanType;
            mesdata.Data = new { VALUE = inputTxt, Station = Cmd.JStation, Input = Cmd.CurrStationInput, ScanType = Cmd.ScanType, Station_NO = Scanda.Id, DisplayName = Scanda.Station };
            ...
            if (JO["Status"].ToString() == "Pass")
            {
                Cmd.JStation = JO["Data"]["Station"]; ;
                //Cmd.Actions.Clear();
                for (int i = Cmd.JStation["StationMessages"].Count(); i > 0; i--)
                {
                    if (Cmd.JStation["StationMessages"][i - 1]["State"].ToString() == "5")
                    {
                        Scanda.SendTextDataToCMC(Cmd.JStation["StationMessages"][i - 1]["Message"].ToString());
                        return;
                    }
                }

                //foreach (JObject jo in Cmd.JStation["StationMessages"])
                //{
                //    if (jo["State"].ToString() == "5")
                //    {
                //        Scanda.SendTextDataToCMC(jo["Message"].ToString());
                //        return;
                //    }
                ...
                Scanda.SendTextDataToCMC(Cmd.JStation["StationMessages"][Cmd.JStation["StationMessages"].Count() - 1]["Message"].ToString());
            }
            else
            {
                Scanda.SendTextDataToCMC(JO["Message"].ToString());
        ...
        public void DoUNDOCMC(CMC503Scanda Scanda, object data)
        {
            ...
            MESAPIData mesdata = new MESAPIData();
            mesdata.Class = "MESStation.Stations.CallStation";
            mesdata.Function = "StationDeleteAP";
            // Cmd.JStation["ScanType"] = Cmd.ScanType;
            mesdata.Data = new { Station = Cmd.JStation, VALUE = inputTxt, ScanType = Cmd.ScanType, Station_NO = Scanda.Id, DisplayName = Scanda.Station };
            ...
            {
                Scanda.SendData(JO["Message"].ToString());
                //comm.toNextAction();
            }
        }
