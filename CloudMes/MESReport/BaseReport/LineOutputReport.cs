...
namespace MESReport.BaseReport
{
    ...
    public class LineOutputReport:ReportBase
    {
        ...
        public override void Run()
        {
            ...
            try
            {
                string sqlline = $@"select line, skuno,sku_name, STATION_NAME,sum(EVENTPASS) pass ,sum(REPAIR_FAILED_FLAG) fail,'{svalue}' as StartTime,'{evalue}' as EndTime from(  
                               select a.line, a.skuno,b.sku_name, a.STATION_NAME, a.REPAIR_FAILED_FLAG INPUTUNIT,CASE WHEN a.VALID_FLAG = '1' AND a.REPAIR_FAILED_FLAG = '0' THEN 1 ELSE 0 END AS EVENTPASS, a.REPAIR_FAILED_FLAG
                    from r_sn_station_detail a left join (select distinct skuno,sku_name from c_sku) b on a.skuno=b.skuno where 1=1"; 
                if (!string.IsNullOrEmpty(skuno))
                {
                    sqlline = sqlline + $@" and a.skuno = '{skuno}'";
                }
                if (line != "ALL")
                {
                    sqlline = sqlline +$@" and a.line = '{line}'";
                }
                if (station != "ALL")
                {
                    sqlline = sqlline + $@"AND a.STATION_NAME = '{station}'";
                }
                sqlline = sqlline + $@"AND a.EDIT_TIME BETWEEN TO_DATE('{svalue}', 'YYYY/MM/DD HH24:MI:SS')
                                AND TO_DATE('{evalue}','YYYY/MM/DD HH24:MI:SS'))
                                GROUP BY line, skuno,sku_name, STATION_NAME order by line";

                if (bu == "VERTIV") //VERTIV多加個UPH nchen 2020-04-01
                {
                    sqlline = @"select aa.line,
       aa.skuno,
       aa.STATION_NAME,
       dd.uph_qty,
       aa.pass,
  ...
  from (" + sqlline;
                    sqlline = sqlline + @") aa
  left join c_line bb
    on aa.line = bb.line_name
  left join (select p_no, line_name, uph_qty
               from (select p_no,
                            line_name,
                            uph_qty,
                            row_number() over(partition by p_no, line_name order by pcas_time desc) rn
                       from mes1.c_pno_uph@vertivapdb) cc
              where cc.rn = 1) dd
    on aa.skuno = dd.p_no
   and bb.line_pcas = dd.line_name
 order by aa.line";
                }
                ...
                for (int i = 0; i < dt.Rows.Count; i++) {
                    ...
                    linkDataRow["EndTime"] = "";
                    if (bu == "VERTIV") linkDataRow["UPH"] = ""; //VERTIV多加個UPH
