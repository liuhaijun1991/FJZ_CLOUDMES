...
{
    public class WoWipStatusReport : ReportBase
    {
        ...
        public override void Run()
        {
            ...
            try
            {
                ...
                #region sqlWO：查詢工單當前WIP數量及所在工站，包含沒掃維修及掃了維修的數據方便後面計算

                #region 舊SQL
                //string sqlWO = $@"select ccs.CUSTOMER_NAME PRODUCTNAME,rwo.WORKORDERNO,rwo.SKUNO,csc.SERIES_NAME CUSTPARTDESC, rwo.WORKORDER_QTY,rwo.DOWNLOAD_DATE StartDate,count(rsn.SN) station_wip,rsn.route_id,rsn.NEXT_STATION,
                //                  rwo.SKU_VER from r_sn rsn, r_wo_base rwo,c_sku csk,c_series csc,c_customer ccs  where rsn.workorderno = rwo.workorderno /*and rsn.valid_flag =1*/ and rsn.REPAIR_FAILED_FLAG = 0 and rwo.closed_flag = '0' 
                //                   and rwo.skuno = csk.skuno and csk.c_series_id = csc.id and csc.customer_id = ccs.id
                //                   Temp_Sql
                //                   group by  ccs.CUSTOMER_NAME,rwo.WORKORDERNO,rwo.SKUNO,csc.SERIES_NAME,rwo.WORKORDER_QTY,rwo.DOWNLOAD_DATE,rsn.route_id,rsn.NEXT_STATION,rwo.SKU_VER
                //                   order by rwo.SKUNO";
                #endregion
                #region 舊SQL
                //string sqlWO = $@"
                ...
                if (dc.Rows[0]["CUSTOMER_NAME"].ToString() == "JUNIPER")
                {
                    sqlWO = $@"select *
                                   from (select ccs.CUSTOMER_NAME PRODUCTNAME,
                                                ...
                                                rwo.WORKORDER_QTY,
                                                rwo.DOWNLOAD_DATE StartDate,
                                                count(distinct rsn.SN) station_wip,
                                                rsn.route_id,
                                                rsn.NEXT_STATION,
                                                rwo.SKU_VER
                                           from 
                                                ...
                                                c_series   csc,
                                                c_customer ccs,
												r_sn       rsn left join  r_pre_wo_head rpw on  rpw.wo=rsn.workorderno
                                          where rsn.workorderno = rwo.workorderno
                                            and rsn.REPAIR_FAILED_FLAG = 0
                                            and rwo.closed_flag = '0'
                                            and rwo.skuno = csk.skuno
                                            and rwo.sku_ver=csk.version
                                            and csk.c_series_id = csc.id
                                            and csc.customer_id = ccs.id
                                            AND rsn.NEXT_STATION NOT LIKE 'REVERSE%'
                                            Temp_Sql
                                          group by ccs.CUSTOMER_NAME,
                                                   rwo.WORKORDERNO,
                                                   rwo.SKUNO,
                                         ...
                                         select ccs.CUSTOMER_NAME PRODUCTNAME,
                                           ...
                                           from r_wo_header rwh,
                                                ...
                                                c_series    csc,
                                                c_customer  ccs,
												r_wo_base   rwo left join  r_pre_wo_head rpw on  rpw.wo=rwo.workorderno
                ...
                string sqlFail = $@"select t1.workorderno,
                       ...
                       nvl(t4.inline, 0) in_line,
                       nvl(t5.rewip, 0) re_wip
                  from (select a.workorderno, nvl(count(distinct a.sn), 0) fail_Total /*工單在生產過程中所有掃Fail的次數,不會去重*/
                          from r_repair_main a
                         where 1 = 1
                           and a.fail_station <> 'COSMETIC-FAILURE'
                         ...
                         group by a.workorderno) t1
                  left join (select a.workorderno, nvl(count(distinct a.sn), 0) checkin /*工單在生產過程中所有掃Fail並掃CheckIn的次數(不管有沒沒有掃CheckOut),不會去重*/
                               from r_repair_main a
                              where 1 = 1
                                and a.fail_station <> 'COSMETIC-FAILURE'
                                and substr(a.sn, 0, 1) not in ('#', '*', '~')
                              ...
                              group by a.workorderno) t2
                    on t1.workorderno = t2.workorderno
                  left join (select a.workorderno, nvl(count(distinct a.sn), 0) checkout /*工單在生產過程中所有掃Fail並掃CheckIn且掃CheckOut的次數,不會去重*/
                               from r_repair_main a
                              where 1 = 1
                                and a.fail_station <> 'COSMETIC-FAILURE'
                                and substr(a.sn, 0, 1) not in ('#', '*', '~')
                              ...
                              group by a.workorderno) t3
                    on t1.workorderno = t3.workorderno
                  left join (select a.workorderno, nvl(count(distinct a.sn), 0) inline /*工單SN當前還在維修且未掃CheckIn的個數*/
                              ...
                              group by a.workorderno) t4
                    on t1.workorderno = t4.workorderno
                  left join (select a.workorderno, nvl(count(distinct a.sn), 0) rewip /*工單SN當前還在維修且已掃CheckIn但沒掃CheckOut的個數*/
                               from r_sn a left join r_repair_main b on a.sn = b.sn
                               -- and b.fail_station <> 'COSMETIC-FAILURE'  /*PM統計總數有差異先把這個條件拿掉 WUQING 20201130 */
                                and substr(b.sn, 0, 1) not in ('#', '*', '~')
                                and substr(b.sn, 0, 2) not in ('RW')                
                        ...
                        for (var j = 0; j < s.Value.Rows.Count; j++)
                        {
                            if (s.Value.Rows[j]["WORKORDERNO"].ToString() == dt.Rows[i]["WORKORDERNO"].ToString())
                            {
                                if (dc.Rows[0]["CUSTOMER_NAME"].ToString() == "JUNIPER")
                                {
                                    ...
                                    dt.Rows[i]["GROUPID"] = s.Value.Rows[j]["GROUPID"].ToString();
